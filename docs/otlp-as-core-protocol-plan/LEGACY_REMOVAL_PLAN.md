...
| T13 | Move `Metric` struct to `#[cfg(test)]` | **NEXT** | T20 done. Remaining production uses: `from_metric_kv` (recorder), `MetricsBuffer` (Batch::Input), GraphQL API filter, proto encode/decode. Eliminate these, then move struct to test-only. 171 `Metric::new` + 136 `from_parts` in test code stay as test convenience. |
