...
impl ToTokens for Format {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        let format_tokens = match self {
            Format::Date => quote! { ::sol_config::validation::Format::Date },
            Format::Time => quote! { ::sol_config::validation::Format::Time },
            Format::DateTime => quote! { ::sol_config::validation::Format::DateTime },
            Format::Duration => quote! { ::sol_config::validation::Format::Duration },
            Format::Email => quote! { ::sol_config::validation::Format::Email },
            Format::Hostname => quote! { ::sol_config::validation::Format::Hostname },
            Format::Uri => quote! { ::sol_config::validation::Format::Uri },
            Format::IPv4 => quote! { ::sol_config::validation::Format::IPv4 },
            Format::IPv6 => quote! { ::sol_config::validation::Format::IPv6 },
            Format::Uuid => quote! { ::sol_config::validation::Format::Uuid },
            Format::Regex => quote! { ::sol_config::validation::Format::Regex },
        };
    ...
    fn to_tokens(&self, tokens: &mut TokenStream) {
        let validation_tokens = match self {
            Validation::KnownFormat(format) => {
                quote! { ::sol_config::validation::Validation::KnownFormat(#format) }
            }
            Validation::Length { minimum, maximum } => {
                let min_tokens = option_as_token(*minimum);
                let max_tokens = option_as_token(*maximum);

                quote! { ::sol_config::validation::Validation::Length { minimum: #min_tokens, maximum: #max_tokens } }
            }
            Validation::Range { minimum, maximum } => {
                let min_tokens = option_as_token(*minimum);
                let max_tokens = option_as_token(*maximum);

                quote! { ::sol_config::validation::Validation::Range { minimum: #min_tokens, maximum: #max_tokens } }
            }
            Validation::Pattern(pattern) => {
                quote! { ::sol_config::validation::Validation::Pattern(#pattern.to_string()) }
            }
        };
